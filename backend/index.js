...
console.log("Environment variables loaded:", {
  ...
  OPENAI_API_KEY_LENGTH: process.env.OPENAI_API_KEY
    ...
    : 0,
  OPENAI_API_KEY_START: process.env.OPENAI_API_KEY
...
app.post("/api/chatgpt", async (req, res) => {
  try {
    ...
    console.log(
      `Attempting ChatGPT API call with model: ${model}, prompt: ${prompt.substring(
        0,
        50
      )}...`
    );
    ...
    if (error.response) {
      if (error.response.status === 401) {
        return res.status(401).json({
          ...
          details:
            "Invalid OpenAI API key. Please check your key or contact server administrator.",
        });
